 ...
 * @since 1.12
 */
@Plugin(description = "A filter to print the object to a configured stream", outputPorts = @OutputPort(name = OPENxtraceBlockingFilter.OUTPUT_PORT_NAME_RELAYED_EVENTS, description = "Provides each incoming object", eventTypes = { Object.class }), configuration = {})
public final class OPENxtraceBlockingFilter extends AbstractFilterPlugin {
	 ...
	 */
	public OPENxtraceBlockingFilter(final Configuration configuration, final IProjectContext projectContext) {
		...
		if (this.active) {
			if (object instanceof MessageTrace) {
				...
				try {
					...
					log.error("Unable to store message trace with ID " + messageTrace.getTraceId() + ".", e);
				}
			}
		}
	...
	public void setTraceQueue(BlockingQueue<Trace> traceQueue) {
